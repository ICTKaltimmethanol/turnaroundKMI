...
@section('content')
        ...
        <div class="max-w-md p-4 bg-center bg-cover bg-no-repeat bg-[url('https://kaltimmethanol.com/themes/methanol/images/slider2_.jpg')] bg-gray-700 bg-blend-multiply rounded-xl shadow-lg overflow-hidden border border-gray-200">
            ...
            {{-- BODY --}}
            <div class="p-4 pt-2  rounded-lg text-center space-y-2">
                {{-- HEADER --}}
                ...
                <div class="text-gray-800 py-2">
                    <h3 class="text-3xl font-bold text-gray-100">{{ $employee->full_name }}</h3>
                    <p class="text-lg text-gray-200 italic">{{ $employee->company->name ?? 'Nama Perusahaan' }} - {{ $employee->position->name ?? 'Posisi' }}</p>
                    <p class="text-md text-gray-300 ">{{ $employee->employees_code }}</p>
                </div>
